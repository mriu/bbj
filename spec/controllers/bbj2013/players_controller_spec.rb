require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe Bbj2013::PlayersController do

  # This should return the minimal set of attributes required to create a valid
  # Bbj2013::Player. As you add validations to Bbj2013::Player, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {}
  end

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # Bbj2013::PlayersController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  describe "GET index" do
    it "assigns all bbj2013_players as @bbj2013_players" do
      player = Bbj2013::Player.create! valid_attributes
      get :index, {}, valid_session
      assigns(:bbj2013_players).should eq([player])
    end
  end

  describe "GET show" do
    it "assigns the requested player as @player" do
      player = Bbj2013::Player.create! valid_attributes
      get :show, {:id => player.to_param}, valid_session
      assigns(:player).should eq(player)
    end
  end

  describe "GET new" do
    it "assigns a new player as @player" do
      get :new, {}, valid_session
      assigns(:player).should be_a_new(Bbj2013::Player)
    end
  end

  describe "GET edit" do
    it "assigns the requested player as @player" do
      player = Bbj2013::Player.create! valid_attributes
      get :edit, {:id => player.to_param}, valid_session
      assigns(:player).should eq(player)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Bbj2013::Player" do
        expect {
          post :create, {:player => valid_attributes}, valid_session
        }.to change(Bbj2013::Player, :count).by(1)
      end

      it "assigns a newly created player as @player" do
        post :create, {:player => valid_attributes}, valid_session
        assigns(:player).should be_a(Bbj2013::Player)
        assigns(:player).should be_persisted
      end

      it "redirects to the created player" do
        post :create, {:player => valid_attributes}, valid_session
        response.should redirect_to(Bbj2013::Player.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved player as @player" do
        # Trigger the behavior that occurs when invalid params are submitted
        Bbj2013::Player.any_instance.stub(:save).and_return(false)
        post :create, {:player => {}}, valid_session
        assigns(:player).should be_a_new(Bbj2013::Player)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Bbj2013::Player.any_instance.stub(:save).and_return(false)
        post :create, {:player => {}}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested player" do
        player = Bbj2013::Player.create! valid_attributes
        # Assuming there are no other bbj2013_players in the database, this
        # specifies that the Bbj2013::Player created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Bbj2013::Player.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, {:id => player.to_param, :player => {'these' => 'params'}}, valid_session
      end

      it "assigns the requested player as @player" do
        player = Bbj2013::Player.create! valid_attributes
        put :update, {:id => player.to_param, :player => valid_attributes}, valid_session
        assigns(:player).should eq(player)
      end

      it "redirects to the player" do
        player = Bbj2013::Player.create! valid_attributes
        put :update, {:id => player.to_param, :player => valid_attributes}, valid_session
        response.should redirect_to(player)
      end
    end

    describe "with invalid params" do
      it "assigns the player as @player" do
        player = Bbj2013::Player.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Bbj2013::Player.any_instance.stub(:save).and_return(false)
        put :update, {:id => player.to_param, :player => {}}, valid_session
        assigns(:player).should eq(player)
      end

      it "re-renders the 'edit' template" do
        player = Bbj2013::Player.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Bbj2013::Player.any_instance.stub(:save).and_return(false)
        put :update, {:id => player.to_param, :player => {}}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested player" do
      player = Bbj2013::Player.create! valid_attributes
      expect {
        delete :destroy, {:id => player.to_param}, valid_session
      }.to change(Bbj2013::Player, :count).by(-1)
    end

    it "redirects to the bbj2013_players list" do
      player = Bbj2013::Player.create! valid_attributes
      delete :destroy, {:id => player.to_param}, valid_session
      response.should redirect_to(bbj2013_players_url)
    end
  end

end
